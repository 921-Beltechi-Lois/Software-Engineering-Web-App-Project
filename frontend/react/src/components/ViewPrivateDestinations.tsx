...
export const DestinationsShowAllPrivate = () => {
  ...
  useEffect(() => {
    ...
    const fetchLoggedInUser = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/loggedusername");
        const loggedInUser = response.data;
        console.log(loggedInUser);
        setUser(loggedInUser);
        console.log(user);

        // Call fetchPrivateDestinations here
        fetchPrivateDestinations(loggedInUser);
      } catch (error) {
        console.log(error);
      }
    };

    const fetchPrivateDestinations = (user: any) => {
      fetch(`http://localhost:8080/api/userdestinations/${user}`)
  ...
  return (
    <Container>
      ...
      {!loading && privateDestinations.length == 0 && (
        <div>No private destinations found</div>
      )}
